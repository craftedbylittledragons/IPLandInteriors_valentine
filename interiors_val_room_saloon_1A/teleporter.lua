...
-----------------------------------------------------
RegisterCommand("takemeto_SaloonRoom1A", function(source, args)    
    if args ~= nil then   
        local data =  source 
        local ped = PlayerPedId() 
        local coords = GetEntityCoords(ped)        
        if Config.TeleportME == true then 
            TriggerEvent( "SaloonRoom1A:scottybeammeup", Config.x, Config.y, Config.z )
        else 
...
RegisterNetEvent('SaloonRoom1A:scottybeammeup')
AddEventHandler('SaloonRoom1A:scottybeammeup', function(x,y,z)  
