...
-----------------------------------------------------
RegisterCommand("takemeto_SaloonRoom1B", function(source, args)    
    if args ~= nil then   
        local data =  source 
        local ped = PlayerPedId() 
        local coords = GetEntityCoords(ped)        
        if Config.TeleportME == true then 
            TriggerEvent( "SaloonRoom1B:scottybeammeup", Config.x, Config.y, Config.z )
        else 
...
RegisterNetEvent('SaloonRoom1B:scottybeammeup')
AddEventHandler('SaloonRoom1B:scottybeammeup', function(x,y,z)  
