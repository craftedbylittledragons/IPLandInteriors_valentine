...
-----------------------------------------------------
RegisterCommand("takemeto_ValSaloon", function(source, args)    
    if args ~= nil then   
        local data =  source 
        local ped = PlayerPedId() 
        local coords = GetEntityCoords(ped)        
        if Config.TeleportME == true then 
            TriggerEvent( "ValSaloon:scottybeammeup", Config.x, Config.y, Config.z )
        else 
...
RegisterNetEvent('ValSaloon:scottybeammeup')
AddEventHandler('ValSaloon:scottybeammeup', function(x,y,z)  
