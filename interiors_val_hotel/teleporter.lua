...
-----------------------------------------------------
RegisterCommand("takemeto_ValHotel", function(source, args)    
    if args ~= nil then   
        local data =  source 
        local ped = PlayerPedId() 
        local coords = GetEntityCoords(ped)        
        if Config.TeleportME == true then 
            TriggerEvent( "ValHotel:scottybeammeup", Config.x, Config.y, Config.z )
        else 
...
RegisterNetEvent('ValHotel:scottybeammeup')
AddEventHandler('ValHotel:scottybeammeup', function(x,y,z)  
