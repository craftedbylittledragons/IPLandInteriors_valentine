...
-----------------------------------------------------
RegisterCommand("takemeto_SaloonRoom2B", function(source, args)    
    if args ~= nil then   
        local data =  source 
        local ped = PlayerPedId() 
        local coords = GetEntityCoords(ped)        
        if Config.TeleportME == true then 
            TriggerEvent( "SaloonRoom2B:scottybeammeup", Config.x, Config.y, Config.z )
        else 
...
RegisterNetEvent('SaloonRoom2B:scottybeammeup')
AddEventHandler('SaloonRoom2B:scottybeammeup', function(x,y,z)  
