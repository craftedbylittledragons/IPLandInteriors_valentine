...
-----------------------------------------------------
RegisterCommand("takemeto_SalooonRoom2A", function(source, args)    
    if args ~= nil then   
        local data =  source 
        local ped = PlayerPedId() 
        local coords = GetEntityCoords(ped)        
        if Config.TeleportME == true then 
            TriggerEvent( "SalooonRoom2A:scottybeammeup", Config.x, Config.y, Config.z )
        else 
...
RegisterNetEvent('SalooonRoom2A:scottybeammeup')
AddEventHandler('SalooonRoom2A:scottybeammeup', function(x,y,z)  
